...
export function registerCustomEventTools(server, client) {
    ...
    server.tool('dwlf_get_custom_event_notifications', 'Get all recently fired custom events where the user has notifications enabled. Returns fires across ALL symbols in one call — the most efficient way to check what custom events have triggered. The eventName field contains the human-readable name (e.g. "wcl_confirmed"). The eventType field is always "custom_event" — use eventName to distinguish events.', {
    ...
    server.tool('dwlf_compile_custom_event', 'Compile a custom event into an executable event definition. Must be called after creating or updating a custom event.', {
    ...
    server.tool('dwlf_update_custom_event', 'Update an existing custom event. Accepts any subset of {name, description, visual, metadata, version, notifyOnFire} via bodyJson. Most common use: flip notifyOnFire to true after creation so future fires surface in dwlf_get_custom_event_notifications. If you change `visual` or any condition shape, re-call dwlf_compile_custom_event afterwards to regenerate the executable.', {
    ...
    server.tool('dwlf_delete_custom_event', 'Delete a custom event permanently. Hard delete — also removes the compiled executable, symbol activations, and watermark state. Past fires already written to EventsTable are NOT deleted (they remain as historical record).', {
