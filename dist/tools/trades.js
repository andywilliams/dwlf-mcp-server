...
export function registerTradeTools(server, client) {
    ...
    // 8. Delete trade
    server.tool('dwlf_delete_trade', 'Delete a trade by ID.', {
        tradeId: z.string().describe('Trade ID'),
    }, async ({ tradeId }) => {
        try {
    ...
    server.tool('dwlf_position_size', 'Calculate recommended position size based on account size, risk percentage, entry price, and stop loss.', {
