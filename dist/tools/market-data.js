...
export function registerMarketDataTools(server, client) {
    ...
    server.tool('dwlf_get_events', 'Get indicator events (cycle lows/highs, crossovers, breakouts, divergences, etc.). ' +
        ...
        try {
            ...
            if (days !== undefined)
                filtersApplied.days = days;
            if (fromDate)
                filtersApplied.fromDate = fromDate;
            if (toDate)
                filtersApplied.toDate = toDate;
            if (noTimeWindow) {
            ...
            if (uniquePerSymbol !== undefined)
                filtersApplied.uniquePerSymbol = uniquePerSymbol;
            if (customEventId)
    ...
    server.tool('dwlf_get_price_picture', 'Aggregate the price-meaningful indicator events for a symbol into a chronological narrative — ' +
        ...
        try {
            ...
            for (const row of projected) {
                ...
                if (existingIdx === undefined) {
                    ...
                    const allLengths = [...existingLengths, row.maLength].filter((n) => Number.isFinite(n));
