---
title: dwlf-mcp-server — engineering standards
profile: lib
---

# dwlf-mcp-server — engineering standards
...
- **FMT-1 · The formatter is the standard** — One committed formatter+linter config per repo, enforced in CI; style (incl. line width) is never argued in review. *(aspirational — no formatter/linter config found, or it is not wired to a lint script or CI job; adopt the tooling or drop this line)*
- **FMT-2 · Small files** — Judged by responsibility first, lines second: warn >640 lines, finding >1040.
...
- **HR-1** (2026-09-10) — A comment states a constraint the code cannot show — the rule that holds NOW, what breaks if the line changes. Three kinds are banned however accurate on the day they were written: (1) HISTORY — "an earlier version did X", "review round N found Y", rationale-by-history; git, the PR body and the ticket own that (C1). (2) DATED SNAPSHOTS — measurements, counts and trace records ("261 orphans as of 1-Sep", "all readers checked 30-Aug"); a frozen fact is stale within days and every later review round pays to re-read it. The number goes on the ticket; the code keeps the constraint that outlives it, plus the DWLF-N pointer. (3) REVIEWER DIALOGUE — pre-rebuttals of alternatives and correctness assertions ("nothing downstream mis-reads this"); that is the PR body talking to a reviewer, and left in source it anchors the next AI review toward trusting prose over verifying code — then misleads with authority the day it drifts. This targets KIND, not length: a long comment stating a live invariant earns its place (COM-1). Evidence: DWLF-127 ran 28 lgtm rounds and 22 of 109 findings were comments gone stale or contradictory; DWLF-104 (SPT #599) and DWLF-123 (jobs #354) shipped production files 64–85% comment lines, the bulk kinds 2–3.
...
## Choices
*The contested-toggle answers from `lgtm standards init` (2026-09-10).*
- 2026-09-10 — **FUN-1** → Balanced (recommended) — cognitive complexity ≤15 primary, length backstop
- 2026-09-10 — **COM-1** → Why-comments encouraged (recommended) — code first, rationale welcome
- 2026-09-10 — **FMT-2** → Yes (recommended) — soft thresholds, responsibility judged first
- 2026-09-10 — **MOD-4** → Balanced (recommended) — flag both over-aggregation and over-sharding
- 2026-09-10 — **G5** → Rule of Three (recommended) — knowledge-dup only, abstraction must be real
- 2026-09-10 — thresholds: function warn >100 / finding >130 lines · file warn >640 / finding >1040 lines
- 2026-09-10 — repo scan at selection time: 25 source files; function lines p50 98 / p95 626 / max 638; file lines p50 170 / p95 631 / max 643; max positional args seen 2 (F1 caps at 3) (approximate scan)
- 2026-09-10 — pre-existing at adoption: ~15 function(s) and ~0 file(s) already exceed these thresholds. Adopted ≠ already true — they become findings only when the code is touched (new-code scope).
