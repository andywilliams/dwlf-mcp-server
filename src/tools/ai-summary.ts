...
export function registerAISummaryTools(
  server: McpServer,
  client: DWLFClient
) {
  // 1. AI Dashboard
  server.tool(
    'dwlf_ai_dashboard',
    'Get a complete account overview in a single call: watchlist with prices, active signals, open trades, portfolios, strategies, and recent events. Ideal for "what\'s going on?" queries. Requires markets:read permission; other sections degrade gracefully based on permissions.',
    {},
    async () => {
      try {
  ...
  server.tool(
    ...
    'Get a condensed single-symbol view: latest price with change, recent candles, key indicators, S/R levels, recent events, and active signals. Perfect for "how\'s BTC doing?" queries.',
    {
