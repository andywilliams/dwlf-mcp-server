...
interface AcademyManifest {
  tracks: AcademyTrack[];
}

// ── Cached manifest ───────────────────────────────────────────────

let cachedManifest: AcademyManifest | null = null;
let cacheTimestamp = 0;

async function getManifest(): Promise<AcademyManifest> {
  const now = Date.now();
  if (cachedManifest && now - cacheTimestamp < MANIFEST_TTL_MS) {
    return cachedManifest;
  }
  const { data } = await axios.get<AcademyManifest>(
    `${ACADEMY_BASE}/manifest.json`,
    { timeout: 15000 }
  );
  cachedManifest = data;
  cacheTimestamp = now;
...
function rewriteImageUrls(markdown: string): string {
  ...
  return markdown.replace(
    /!\[([^\]]*)\]\((?:\.\/)?assets\//g,
    `![$1](${ACADEMY_BASE}/assets/`
  );
}

// ── Tool registration ─────────────────────────────────────────────

export function registerAcademyTools(server: McpServer) {
  ...
  server.tool(
    ...
    async ({ query }) => {
      try {
        ...
        const results: Array<{
          track: { id: string; title: string; description: string };
          matchedLessons: AcademyLesson[];
          trackMatch: boolean;
        }> = [];
        ...
        for (const track of manifest.tracks) {
          ...
          const matchedLessons = track.lessons.filter((l) =>
